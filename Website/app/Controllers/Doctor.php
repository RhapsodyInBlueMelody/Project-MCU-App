...
class Doctor extends AuthenticatedController
{
    ...
    {
        parent::__construct("doctor", "auth/doctor/login");
            ...
            if ($existingUser) {
                ...
                if ($existingUser["role"] != $role) {
                    return redirect()
                        ...
                        ->with(
                            "error",
                            "Email ini sudah terdaftar dengan peran yang berbeda."
                        );
                }

                // Check if this is a doctor and if they're verified
                if ($role == "doctor") {
                    $dokterModel = new \App\Models\DokterModel();
                    $doctor = $dokterModel
                    ...
                    if (!$doctor) {
                        return redirect()
                            ->to("auth/doctor/login")
                            ->with("error", "Akun dokter tidak ditemukan.");
                    }
                    ...
                    if ($doctor["verification_status"] === "pending") {
                        return redirect()
                            ...
                            ->with(
                                "error",
                                "Akun dokter Anda sedang dalam proses verifikasi."
                            );
                    }
                    ...
                    if ($doctor["verification_status"] === "rejected") {
                        return redirect()
                            ...
                            ->with(
                                "error",
                                "Verifikasi akun dokter Anda ditolak. Alasan: " .
                    ...
                    if ($doctor["is_verified"] == 0) {
                        return redirect()
                            ...
                            ->with(
                                "error",
                                "Akun dokter Anda belum diverifikasi."
                            );
                    }
                }
                ...
                if ($role == "doctor") {
                    ...
                    // Redirect to a page to collect additional doctor information
                    return redirect()->to("auth/register/social/doctor");
                } else {
                    ...
                    $newUserData["user_id"] = $userModel->getInsertID();

                    // Create patient profile if it's a patient
                    if ($role == "patient") {
                        $patientModel = new \App\Models\PatientModel();
                        $patientData = [
                            "PASIEN_ID" => $newUserData["user_id"],
                            "NAMA_LENGKAP" => $userData->name,
                            "EMAIL" => $userData->email,
                            "created_by" => $newUserData["user_id"],
                            "updated_by" => $newUserData["user_id"],
                        ];
